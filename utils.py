...
def select_random_points(pr, gt, point_num = 9, status='train'):
    """
    ...
    for j in range(error.shape[0]):
        ...
        for i in selected_indices:
            ...
            # else:
            #     label = 0
            if status=='train' and random.random() < 0.1:
                label = 1-label
...
def init_point_sampling(mask, get_point=1, status='train'):
    """
    ...
    if get_point == 1:
        ...
        if fg_size > 10:
            ...
            # num_fg = int((fg_size/bg_size)*5*get_point)
            # num_bg = max(get_point - num_fg, 1)
            ...
            bg_indices = np.random.choice(bg_size, size=get_point, replace=True)
            bg_coords = bg_coords[bg_indices]
            coords = np.concatenate([bg_coords], axis=0)
            labels = np.concatenate([np.zeros(get_point)]).astype(int)
            indices = np.random.permutation(get_point)
            coords, labels = torch.as_tensor(coords[indices], dtype=torch.float), torch.as_tensor(labels[indices], dtype=torch.int)
...
def valid_transforms(img_size, ori_h, ori_w):
    transforms = []
    if ori_h < img_size and ori_w < img_size:
        transforms.append(A.PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT, value=(0, 0, 0)))
    else:
...
def generate_point(masks, labels, low_res_masks, batched_input, point_num, status='valid'):
    ...
    points, point_labels = select_random_points(masks_binary, labels, point_num = point_num, status=status)
...
def generate_point_self(masks, labels, low_res_masks, batched_input, point_num, status='train'):
    masks_clone = masks.clone()
    if masks_clone.max() < 2:
        masks_binary = masks_clone
    else:
        masks_sigmoid = torch.sigmoid(masks_clone)
        masks_binary = (masks_sigmoid >= 0.5).float()
    ...
    for mask_binary in masks_binary:
        try:
            point, point_label = init_point_sampling(mask_binary[0], point_num, status)
            temp_img = 255*mask_binary[0].cpu().numpy()
            point_coords_list.append(point)
            point_labels_list.append(point_label)
            # cv2.imwrite('./temp/tt_good.png', temp_img)
        except Exception as e:
            # print(e)
            temp_img = 255*mask_binary[0].cpu().numpy()
            cv2.imwrite('./temp/tt_pool.png', temp_img)
            print(batched_input['mask_path'])
            pass
            
            
        
    points = torch.stack(point_coords_list, dim=0)
    ...
    if visual_prompt: #visualize the prompt
        ...
        if points is not None:
            ...
            for point, label in zip(ori_points, point_labels):
                ...
                mask[y, x] = color
                cv2.drawMarker(mask, (x, y), color, markerType=cv2.MARKER_CROSS , markerSize=5, thickness=1)  
    os.makedirs(save_path, exist_ok=True)
    mask_path = os.path.join(save_path, f"{mask_name}")
    cv2.imwrite(mask_path, np.uint8(mask))


def save_masks_unet(preds, save_path, mask_name):

    preds = torch.sigmoid(preds[0])
    preds[preds >= 0.5] = int(1)
    preds[preds < 0.5] = int(0)

    mask = preds.squeeze().cpu().numpy()*255
    # mask = cv2.cvtColor(mask * 255, cv2.COLOR_GRAY2BGR)

    os.makedirs(save_path, exist_ok=True)

    for i, temp_mask in enumerate(mask):
        mask_path = os.path.join(save_path, f"{mask_name.split('.')[0]}"+'_'+str(i+1) + '.png')
        cv2.imwrite(mask_path, np.uint8(temp_mask))
...
class DiceLoss(nn.Module):
    ...
    def forward_cls(self, pred, mask):
        """
        pred: [B, 1, H, W]
        mask: [B, 1, H, W]
        """
    ...
    def forward(self, pred, mask):
        ...
        loss = 0
        for i in range(c):
            loss += self.forward_cls(pred[:,i:i+1,:,:], mask[:,i:i+1,:,:])
        loss_avg = loss
        return loss_avg
