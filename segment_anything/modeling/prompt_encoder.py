...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        self.cls_embed = nn.Linear(1, embed_dim)
    ...
    def forward(
        ...
        """
        # bs = self._get_batch_size(points, boxes, clss)
        # sparse_embeddings = torch.empty((bs, 0, self.embed_dim), device=self._get_device()) #B,0,256  空[]

        # ## cls embedding
        # if clss != None:
        #     temp_cls = self.cls_embed(clss.view(bs, 1).float()).view(bs,1,-1)
        #     sparse_embeddings = torch.cat([sparse_embeddings, temp_cls], dim=1)
