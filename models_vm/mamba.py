...
def selective_scan_easy(us, dts, 
                        As, Bs, Cs, Ds, 
                        delta_bias=None, 
...
def selective_scan_easyv3(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64):
    ...
    def cut_chunk(us, dts, Bs, Cs, chunksize=chunksize):
        ...
        _pad = lambda x: torch.nn.functional.pad(x.reshape(B * H, L, -1), (0,0,0,to_pad,0,0)).view(B * H, NT, chunksize, x.shape[-1])
    ...
    def ss_chunk_h1y1(qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=_mask_tril, scale=1):
        ...
        q_mul_w = qs[...,None] * (ws+1e-9) * scale
        k_div_w = ks[...,None] / (ws+1e-9)

        if torch.isnan(k_div_w).any():
            print('nan')
            
        if torch.isinf(k_div_w).any():
            print('inf')
