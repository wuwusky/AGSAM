...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="m1", help="run model name")
    ...
    parser.add_argument("--model_name", type=str, default="FCN", help="model names [FCN, DeepLabv3, PSPNet, Fast-SCNN, SegFormer, Mask2Former, TGANet, Unet]")
...
def eval(args, model_sam):
    ...
    temp_save_dir = os.path.join(args.work_dir, "results", str(args.num_sample), args.model_name+'_m1')
    os.makedirs(temp_save_dir, exist_ok=True)
    save_path_json = os.path.join(temp_save_dir, f"temp_{args.model_name+'_m1'}_fold{str(args.fold)}.json")
...
if __name__ == '__main__':
    ...
    if args.train=='true':
        ...
        for temp_name in list_model_names:
            args.model_name = temp_name
