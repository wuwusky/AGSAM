...
def train_one_epoch(args, model, optimizer, train_loader, criterion, model_agent, optimizer_agent, optimizer_agent_prompt):
    ...
    for batch, batched_input in enumerate(train_loader):
        ...
        sparse_embeddings = out['points_prompt'].view(b*3, -1, 256) 
        dense_embeddings = out['masks_prompt'].view(b*3, 256, h//16, w//16)
        ...
        for i in range(0,b*c,3):
            list_res_masks.append(low_res_masks[i:i+3,0,:,:])
        ...
        # loss_sam = 0
        # for m in masks:
        #     # mask_fusion = mask_sam*(1-args.fusion_ratio) + m*args.fusion_ratio
        #     mask_fusion = F.sigmoid(mask_sam)*(1-args.fusion_ratio) + F.sigmoid(m)*args.fusion_ratio
        #     # mask_fusion = F.sigmoid(mask_sam)*(1-0.1) + F.sigmoid(m)*0.1  # number<16
        ...
        if args.num_sample>1 or args.num_sample<0:
            if (batch+1)%1==0:
        ...
        # if batch % 1 == 0:
        #     with torch.no_grad():
        #         out = model_agent(image_input, image_embeddings_ori)
        #         if args.model_name == 'Unet' or args.model_name == 'Unet_sam' or args.model_name == 'Unet_sam_joint':
        #             masks = out['masks']
        #         elif args.model_name == 'FCN' or args.model_name == 'DeepLabV3' or args.model_name == 'PSPNet' or args.model_name == 'Fast-SCNN' or args.model_name == 'TGANet':
        #             mask = out['out']
        ...
        #     mask_fusion = F.sigmoid(mask_sam)*(1-args.fusion_ratio) + F.sigmoid(m)*args.fusion_ratio
        #     loss_demask = criterion(mask_fusion, labels)

        #     loss_demask.backward(retain_graph=True)
        ...
        train_loader.set_postfix(loss_agent=loss_mask.item(), loss_sam=loss_sam.item())
...
def train(args):
    ...
    if args.model_name == 'Unet':
        model_agent = NestedUNet(3, 3, True, True).to(args.device)
    elif args.model_name == 'FCN':
        model_agent = fcn_resnet50_my(num_classes=3, model_type='m3').to(args.device)
    elif args.model_name == 'DeepLabv3':
        model_agent = deeplabv3_resnet50(num_classes=3).to(args.device)
    elif args.model_name == 'PSPNet':
        model_agent = PSPNet(n_classes=3).to(args.device)
    elif args.model_name == 'Fast-SCNN':
        model_agent = FastSCNN(num_classes=3).to(args.device)
    elif args.model_name == 'SegFormer':
        model_agent = Segformer().to(args.device)
    elif args.model_name == 'TGANet':
        model_agent = TGAPolypSeg(num_classes=3).to(args.device)
    
    optimizer_agent = optim.AdamW(model_agent.parameters(), lr=args.lr, weight_decay=5e-4)
    optimizer_agent_prompt = optim.AdamW(itertools.chain(model_agent.mask_prompt.parameters(),model_agent.point_prompt.parameters()), lr=args.lr*0.1, weight_decay=5e-4)
    ...
    for epoch in range(0, args.epochs):
        ...
        valid_iter_metrics,_ = eval_one_epoch(args, model, valid_loader, model_agent)
        ...
        if temp_metic >= best_metric and float(train_metrics['dice'])>0.9:
    ...
    temp_metic = (float(test_metrics['dice_1'])+float(test_metrics['dice_2'])+float(test_metrics['dice_3']))/3
...
def eval(args):
    ...
    if args.model_name == 'Unet':
        ...
        model_agent = deeplabv3_resnet50_my(num_classes=3).to(args.device)
    elif args.model_name == 'PSPNet':
        model_agent = PSPNet(n_classes=3).to(args.device)
    elif args.model_name == 'Fast-SCNN':
        model_agent = FastSCNN(num_classes=3).to(args.device)
...
if __name__ == '__main__':
    ...
    if args.train=='true':
        ...
        args.metrics = ['dice', 'dice_1','dice_2','dice_3', 'hd_1', 'hd_2', 'hd_3']
        eval(args)
