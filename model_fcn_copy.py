...
class _SimpleSegmentationModel(nn.Module):
        ...
        if self.model_type!='m0':
            ...
            if self.model_type == 'm3':
                ...
                self.mask_prompt = nn.Sequential(
                    nn.AvgPool2d(3,2,1),
                    aug_conv(2048, base*2, 2, False),
                    aug_conv(base*2, base*4, 2, False),
                    aug_conv(base*4, base*8, 2, False),
                    aug_conv(base*8, base*16, 1, False),
                    ...
                    aug_conv(base*16, 768, 1, False),
                    nn.Conv2d(768, 768, 1, 1, 0),
    ...
    def forward(self, x: Tensor, input_sam=None) -> Dict[str, Tensor]:
        ...
        if self.model_type != 'm0':
            ...
            feat_com = torch.cat([feat, feat_sam], dim=1)
            x = self.feat_fusion(feat_com)
            out = self.classifier(x)
            if self.model_type == 'm3':
                temp_x = F.interpolate(x, size=input_shape, mode="bilinear", align_corners=False)
                result['masks_prompt'] = self.mask_prompt(temp_x)
